...
export default class Postings extends Component {
  ...
  async handleDeletePosting(postingID) {
    await axios
      .post(`${endpoint}/jobs/${postingID}/delete`)
      .then((res) => {
            ...
            <tbody>
              {this.state.postings.map((posting, index) => (
                <tr className="admin-table-row" key={index}>
                  ...
                  <td style={{ width: '15%' }}>
                    <div className="admin-careers-row-btn">
                      <Button
                        variant="success"
                        href={`/admin/careers/${posting._id}`}
                        style={{ width: '100%' }}
                      >
                      ...
                      <Button
                        variant="danger"
                        onClick={() => this.handleDeletePosting(posting._id)}
                        style={{ width: '100%' }}
                      >
