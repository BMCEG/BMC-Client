...
export default class Courses extends Component {
  ...
  async handleDeleteCourse(courseID) {
    await axios
      .post(`${endpoint}/courses/${courseID}/delete`)
      .then((res) => {
  ...
  async componentDidMount() {
    await axios
      .get(`${endpoint}/courses?company=all`)
      .then((res) => {
        this.setState({
          courses: res.data,
        });
            ...
            <tbody>
              {this.state.courses.map((course, index) => (
                <tr className="admin-table-row" key={index}>
                  <td>{index + 1}</td>
                  <td width="10%">
                    <h5>{course.displayTitle}</h5>
                  </td>
                  <td width="10%">
                    <h5>{course.company}</h5>
                  </td>
                  <td width="10%">
                    <h5>
                  ...
                  <td style={{ width: '15%' }}>
                    <div className="admin-contacts-row-btn">
                      ...
                      <Button
                        variant="danger"
                        onClick={() => this.handleDeleteCourse(course._id)}
                        style={{ width: '100%' }}
                      >
