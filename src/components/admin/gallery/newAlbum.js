...
export default class NewAlbum extends Component {
  ...
  async handleAlbumCreate() {
    ...
    await axios
      .post(`${endpoint}/albums/create`, newAlbum)
      .then((res) => {
        ...
        >
          <div className="media-create-form">
            ...
            <div className="trainer-btn-base">
              <Button variant="danger" onClick={this.handleAlbumCreate}>
                Create Album
              </Button>
            </div>
          </div>
              ...
              <tbody>
                {this.state.pictures.map((picture, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <h5 className="inner-text">{picture.url}</h5>
                    </td>
                    <td style={{ width: '15%' }}>
                      <div className="admin-contacts-row-btn">
                        {this.state.media.includes(picture) ? (
                          <Button
                            variant="danger"
                            onClick={() => this.handleRemoveMedia(picture)}
                            style={{ width: '100%' }}
                          >
                          ...
                          <Button
                            variant="success"
                            onClick={() => this.handleAddMedia(picture)}
                            style={{ width: '100%' }}
                          >
