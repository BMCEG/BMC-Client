...
export default class Contacts extends Component {
    ...
    async componentDidMount() {
        await axios.get(`${endpoint}/requests/`).then((res) => {
            this.setState({
                requests: res.data
            })
        }).catch((err) => {
                            ...
                            <tbody>
                                {this.state.requests.map((request, index) => (
                                    <tr key={index}>
                                        ...
                                        <td><h5>{request.email}</h5></td>
                                        <td><h5>{request.mobile}</h5></td>
                                        ...
                                        <td style={{ width: '7vw' }}><div className='admin-contacts-row-btn'>
                                            <Button variant='danger' href={`/admin/contacts/${request._id}`}>Go to request</Button>
                                        </div>
                                        </td>
                                    </tr>
                                ))}
