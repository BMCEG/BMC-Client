...
export default class Coursepage extends Component {
  ...
  async componentDidMount() {
    await axios
      .get(`${endpoint}/courses?courseName=${this.state.courseName}`)
      .then((res) => {
        this.setState({
          course: res.data,
    ...
    await axios
      .get(
        `${endpoint}/applications?applicantUsername=${this.state.applicantUsername}&&courseName=${this.state.courseName}`
      )
      .then((res) => {
        this.setState({
  ...
  render() {
    return (
      <div>
        <h2>
          Hello {this.state.application.firstName}
        ...
        {this.state.courseProjects.map((project, index) => (
          <Button
            key={index}
            onClick={() => this.handleProjectDownload(project.url)}
          >
