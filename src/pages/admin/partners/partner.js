...
export default class Course extends Component {
    ...
    componentDidMount = async () => {
        await axios.get(`${endpoint}/partners/${this.state.partnerID}`)
            .then((res) => {
    ...
    async handleDelete() {
        await axios.post(`${endpoint}/partners/${this.state.partnerID}/delete`)
            .then((res) => {
    ...
    async handleConfirm() {
        ...
        await axios.post(`${endpoint}/partners/${this.state.partner._id}/edit`, editedPartner)
            .then((res) => {
                        ...
                        <Paper className='admin-course-post-paper'>
                            {this.state.isEditing ?
                                <Zoom in={this.state.isEditing}>
                                    <>
                                        <h4>Name: <TextField onChange={this.handleInputChange} value={this.state.editedName} name='editedName' /></h4>
                                        <h5>Url: <TextField onChange={this.handleInputChange} value={this.state.editedURL} name='editedURL' /></h5>
                                        <hr></hr>
                                        <h5>Description:</h5>
